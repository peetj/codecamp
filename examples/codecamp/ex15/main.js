...
/* Initialisation */
_scoreDisplay = Crafty.e("2D, Canvas, Text")
  .attr({ x: 650, y: 20 })
  .text("SCORE: 000000").textColor('#FF0000').textFont({ size: '18px', weight: 'bold', family: 'Courier New' });
resetLives();
...
Crafty.bind("EnterFrame", function() {
    ...
    if (Crafty.frame() % 25 === 0) {
      ...
      if(willShoot){
        ...
        do{
          enemyToShoot = enemies[Math.ceil(Math.random() * 10) - 1];
        }
        while(enemyToShoot && enemyToShoot.id === -1);
...
function decideIfWeWillShoot(){
  ...
  return testNum % 2 === 0 || testNum % 3 === 0 || testNum % 5 === 0;
}
...
function spawnEnemy(offset) {
  ...
  .bind("explode", function() {
      ...
      /* Test for Game over */
      _gameOver = getNumberOfAliensLeft() === 0;
      if(_gameOver)
        window.setTimeout(displayGameOver, 250);
  })
...
function resetLives(){
  if(_livesArr && _livesArr.length > 0){
    for(var j=0; j < _livesArr.length; j++){
      _livesArr[j].destroy();
    }
  }
  for(var i=0; i < _numLivesLeft; i++){
    var ship = Crafty.e("2D, DOM, Image").image("img/ship-mini.png").attr({x:10 + (i*22) , y:20 , w:22 , h:20 });
    _livesArr.push(ship);
  }
}

function handleShipDied(){
  _numLivesLeft -= 1;
  resetLives();
  /* Is this the end? */
  if(_numLivesLeft === 0){
    ...
    /* GAME OVER */
    window.setTimeout(displayGameOver, 750);
    _gameOver = true;
    return;
  }
  window.setTimeout(spawnShip, 1500);
}

function updateScore(amt){
  _currentScore += amt;
  _scoreDisplay.text('SCORE: '+pad(_currentScore,6));
}
...
function getNumberOfAliensLeft(){
  var count = 0;
  for(var i=0; i < enemies.length; i++){
    if(enemies[i].id !== -1){
      count++;
